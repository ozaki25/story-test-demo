...
<Meta title="解説/7. 構成と設計判断" />

# 構成と設計判断

大規模化したときに壊れないことを優先して選んだ構成です。
それぞれ「なぜそうしたか」と「そうしないと何が起きるか」を書いています。
...
実際に踏んだ問題です。詳細は「5. 落とし穴」を参照してください。
...
「Storybook では動くのにテストだけ落ちる」が多発します。
コンポーネントが増えるほど原因の特定が面倒になるので、
**その配線自体を検証するテストを 1 本置いています**
...
Story が増えるほど事故が増えるので、Story は標準の書き方に統一し、
差分は setup で埋めています。
...
Storybook もテストも同じ設定を読みます。設定を 1 ファイルにまとめた効果がここに出ます。

バリアントごとのクラスはオブジェクトへ切り出してください。
...
play 関数の await 漏れのような Story 固有の間違いは検出できないので、
そこを重視するなら ESLint を併用する判断もあります。

なお oxlint は導入直後に、手法④のテストを
「アサーションのないテスト」として指摘してきました。
その対処は「5. 落とし穴」に書いています。
手法の選択が周辺のツール設定にも波及する、という例です。
